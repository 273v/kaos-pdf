...
| `test3.pdf` | Anonymous public scanned PDF, 32 pages, Producer "Xerox WCP C2636" (Xerox WorkCentre Pro scanner), CreationDate 2009-06-18. Native text layer is empty — image-only scan inherited from `kelvin_pdf/tests/resources/test3.pdf`. Surfaced from a public web source during the original kelvin-pdf vendoring (2025); the specific upstream URL was not recorded and is not recoverable from the file metadata. The 2009 scanner-timestamp suggests a public document scanned and posted in the late-2000s — most plausibly a court filing, municipal record, or other public-records document of the era. Per repository policy, no customer / privileged content is in this fixture. | Public document (specific source not recoverable). Used here under fair use for parser-regression testing of the OCR / image-extraction path; the file is treated as opaque image-only input. | 2026-03-27 | `b4eaee56996a4c7b5ae5be5f94feb83d84937f318f0e6bcd16629ea344cc4ea5` | Best-guess provenance — if a known-source scanned fixture is wanted, replace with a scanned EDGAR exhibit or PACER scanned filing. |
...
## Confirmed-source vs. best-guess count

- Confirmed source: **16 / 17**
- Best-guess (specific upstream URL not recoverable): **1 / 17** (`test3.pdf` — anonymous 2009-vintage public scan inherited from `kelvin_pdf`; documented under fair-use for OCR-path parser-regression testing)
