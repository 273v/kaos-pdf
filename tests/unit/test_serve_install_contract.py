"""Pins the install contract for `kaos-pdf-serve`.

audit-04 F-001: `pyproject.toml` advertises a `[mcp]` extra and the
`kaos-pdf-serve` console script imports `kaos_mcp` at runtime. When
that extra is not installed the entry point must exit non-zero with
an error message that names the canonical extra so users have a
...
def test_serve_main_exits_with_mcp_extra_hint_when_kaos_mcp_missing(
    ...
    install extra (``pip install kaos-pdf[mcp]``) rather than naming
    ``kaos-mcp`` directly, so the message tracks the declared
