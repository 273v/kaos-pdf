...
These standards apply to Python code in `kaos-pdf`.

`kaos-pdf` publishes the `kaos_pdf` import package. This package publishes CLI entry point(s): `kaos-pdf`, `kaos-pdf-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-pdf` publishes import package `kaos_pdf`.
- Declare the public API in `kaos_pdf.__all__` when the package has
...
- Names exported from `kaos_pdf.__all__`.
...
- Put optional integrations in named extras. Public extras currently declared: `nlp`, `ocr`, `tables`.
